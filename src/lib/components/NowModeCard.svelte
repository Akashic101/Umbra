...
const nowState = $derived(getEclipseNowState(contacts, nowMs));
	...
	<Card class="w-full p-2 max-w-none col-span-2 lg:col-span-4" size="xl">
		...
		>
			<div class="min-w-0 flex-1 space-y-3">
				...
				>
					{formatNowPhase(nowState.phase, localType)}
				</p>
				<time
				...
				<div class="grid gap-3 sm:grid-cols-2" aria-live="polite">
						...
						<div>
							<p class="text-xs text-gray-500 dark:text-gray-400">
								{m.nowCentralRemaining({
									centralWord: formatCentralWord(localType),
								})}
							</p>
							<p
								class="text-2xl font-semibold tabular-nums text-gray-900 dark:text-white sm:text-3xl"
					...
					{:else if nowState.remainingToNextMs != null && nowState.nextKey}
						<div>
							<p class="text-xs text-gray-500 dark:text-gray-400">
								{m.nowUntilNext({
									label: formatContactLabel(nowState.nextKey, localType),
								})}
							</p>
							<p
								class="text-2xl font-semibold tabular-nums text-gray-900 dark:text-white sm:text-3xl"
			...
			{#if diskSample}
				<div class="mx-auto shrink-0 sm:mx-0">
					<CoverageDisk
						size="md"
						obscuration={diskSample.obscuration}
						magnitude={diskSample.magnitude}
						moonSunRatio={diskSample.moonSunRatio}
						localType={diskSample.localType ?? localType}
						positionAngleDeg={diskSample.moonPaDeg}
					/>
				</div>
			{/if}
		</div>
