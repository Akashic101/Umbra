...
<div class="relative">
	<Button color="alternative" size="sm" class="gap-1.5" aria-label="Favorites">
	...
	>
		<DropdownHeader>
			<span class="block text-sm font-medium">Saved eclipses</span>
		</DropdownHeader>
		{#if appState.sortedFavorites.length === 0}
			<div class="px-4 py-3 text-sm text-gray-500 dark:text-gray-400">
				Save an eclipse and location from Details or More info to reopen it
				here.
			</div>
		{:else}
